function pathlen = gr_calculatePathlengthWs(cfg, connectivity)
% Computes characteristic path length and related graph-distance measures
% from a PLI connectivity struct.
%
% Ported from gr_calculatePathlengthWs.m (which took a raw N x N x m
% adjacency stack and an edgeType flag, and called
% distance_bin/distance_wei + charpath directly). This version keeps the
% same core computation but is wrapped in the cfg-based interface used by
% the other network-metric steps (bv_calculateStrength, bv_calculateSWP,
% gr_calculateQModularity, gr_calculateClusteringWs), so it can plug into
% networkMetrics_standard.m the same way: per condition and per frequency
% band, for the whole network ('Global') and/or for each ROI.
%
% Unlike the other metrics, charpath returns several related measures off
% a single shortest-path computation. cfg.measures selects which of them
% to compute (default: all). Rather than widening the table with a column
% per measure per band (which would blow up fast: nMeasures x nBands
% columns), each selected measure is stacked as its own row via a
% ``Measure`` column, keeping the table in the same tidy/long format
% already used for Region and Condition - band stays the only wide axis,
% consistent with the other metric tables.
%
...
% dropped, and the distance matrix is computed with distance_wei (weighted,
% on weight_conversion(W,'lengths'), matching the original 'weighted'
% edgeType branch) or distance_bin (binary/mst, on the thresholded
% matrix), then passed to charpath. Degenerate graphs (< 3 nodes /
% all-zero) yield NaN for every measure. Per-matrix failures are caught
% and yield NaN with a warning so the pipeline does not stop.
%
% NOTE: distance_bin/distance_wei/weight_conversion/charpath are used from
% FieldTrip's bundled Brain Connectivity Toolbox copy
% (fieldtrip/external/bct) via ft_hastoolbox('BCT', 1), rather than
% bundling a separate copy. ROI-level path length is computed on small
...
%     cfg.measures (cell array of str): Which charpath measures to
%         compute, any of ``'L'`` (characteristic path length),
%         ``'efficiency'``, ``'eccentricity'``, ``'radius'``, ``'diameter'``.
%         Default: all five.
%     cfg.computeGlobal (str): ``'yes'``/``'no'`` whole-network path length
%         (default ``'yes'``).
%     cfg.computeROI (str): ``'yes'``/``'no'`` per-ROI path length
%         (default ``'yes'``).
...
%     cfg.edgeType (str): ``'weighted'`` (weight_conversion + distance_wei)
%         or ``'binary'``/``'mst'`` (thresholds at > 0 and uses
%         distance_bin, matching the original gr_calculatePathlengthWs
%         behaviour). Default ``'weighted'``.
...
%
% Returns:
%     pathlen (struct): Struct with fields ``table`` (columns Subject,
%         Wave, Condition, Region, Measure, plus one column per frequency
%         band), ``conditions``, ``ROI``, ``measures``, and ``bands``.
%         Empty (``[]``) if output already exists and ``cfg.overwrite`` is
%         ``'no'``.

%% ensure distance_bin/distance_wei/weight_conversion/charpath (FieldTrip's bundled BCT) is on the path
ft_hastoolbox('BCT', 1);
...
%% valid measures and their display labels
validMeasures    = {'L', 'efficiency', 'eccentricity', 'radius', 'diameter'};
measureDisplay   = containers.Map( ...
    {'L', 'efficiency', 'eccentricity', 'radius', 'diameter'}, ...
    {'L', 'Efficiency', 'Eccentricity', 'Radius', 'Diameter'});
...
measures             = ft_getopt(cfg, 'measures', validMeasures);
...
if ~computeGlobal && ~computeROI
    error('gr_calculatePathlengthWs: both computeGlobal and computeROI are off; nothing to compute')
end

if isempty(measures)
    error('gr_calculatePathlengthWs: cfg.measures must not be empty')
end
unknownMeasures = setdiff(measures, validMeasures);
if ~isempty(unknownMeasures)
    error('gr_calculatePathlengthWs: unknown cfg.measures %s (valid: %s)', ...
        strjoin(unknownMeasures, ', '), strjoin(validMeasures, ', '))
end

if ~isempty(conditionLabelsList) && length(conditionLabelsList) ~= length(conditions)
    error(['gr_calculatePathlengthWs: cfg.conditionLabels must have the same number of ' ...
...
if computeROI
    if ~quiet
        fprintf(['\t note: ROI-level path length is computed on small subgraphs ' ...
...
%% compute path-length measures per condition x region x measure x band
Subject   = {};
Wave      = {};
Condition = {};
Region    = {};
...
if ~quiet; fprintf('\t calculating path length measures ... '); end
...
for c = 1:numel(condList)
    ...
    for r = 1:numel(regionNames)
        ...
        measurePerBand = struct( ...
            'L',            nan(1, nfreq), ...
            'efficiency',   nan(1, nfreq), ...
            'eccentricity', nan(1, nfreq), ...
            'radius',       nan(1, nfreq), ...
            'diameter',     nan(1, nfreq));
        ...
        for b = 1:nfreq
            ...
            try
                switch edgeType
                    case 'weighted'
                        D = distance_wei(weight_conversion(W, 'lengths'));
                    case {'binary', 'mst'}
                        D = distance_bin(W > 0);
                end
                [L, eff, ecc, rad, diam] = charpath(D);

                measurePerBand.L(b)            = L;
                measurePerBand.efficiency(b)   = eff;
                measurePerBand.eccentricity(b) = mean(ecc);
                measurePerBand.radius(b)       = rad;
                measurePerBand.diameter(b)     = diam;
            catch ME
                warning('gr_calculatePathlengthWs:%s %s/%s band %s: %s', ...
        ...
        for m = 1:numel(measures)
            ...
            Measure{end+1,1}   = measureDisplay(measures{m});     %#ok<AGROW>
            band_vals(end+1,:) = measurePerBand.(measures{m});    %#ok<AGROW>
        end
    end
end
...
T = table(Subject, Wave, Condition, Region, Measure, ...
    'VariableNames', {'Subject', 'Wave', 'Condition', 'Region', 'Measure'});
for b = 1:nfreq
    bname   = bands{b};
...
pathlen.table      = T;
pathlen.conditions = conditions;
pathlen.ROI        = ROI;
pathlen.measures   = measures;
pathlen.bands      = bands;
...
if strcmpi(saveData, 'yes')
    ...
    if ~quiet
        ...
        save(subjectdata.PATHS.(fieldname), 'pathlen')
        fprintf('done \n')
...
end % function gr_calculatePathlengthWs
