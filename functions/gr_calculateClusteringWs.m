function clus = gr_calculateClusteringWs(cfg, connectivity)
% Computes the (mean) clustering coefficient from a PLI connectivity struct.
%
% Ported from unused/gr_calculateClusteringWs.m (which took a raw
% N x N x m adjacency stack and an edgeType flag, and called
% clustering_coef_bu/clustering_coef_wu directly). This version keeps the
...
% gr_calculateQModularity), so it can plug into networkMetrics_standard.m
% the same way: per condition and per frequency band, for the whole
% network ('Global') and/or for each ROI, returned as a tidy table.
%
...
% dropped, and the mean clustering coefficient is computed with
% clustering_coef_wu (weighted, on the gr_normalizeW-normalised matrix,
% matching the original 'weighted' edgeType branch) or clustering_coef_bu
% (binary/mst, on the thresholded matrix). Degenerate graphs (< 3 nodes /
% all-zero) yield NaN. Per-matrix failures are caught and yield NaN with a
% warning so the pipeline does not stop.
%
% NOTE: clustering_coef_bu/clustering_coef_wu are used from FieldTrip's
% bundled Brain Connectivity Toolbox copy (fieldtrip/external/bct) via
% ft_hastoolbox('BCT', 1), rather than bundling a separate copy. ROI-level
% clustering is computed on small subgraphs and should be interpreted
% with caution.
%
% Args:
...
%     cfg.computeGlobal (str): ``'yes'``/``'no'`` whole-network clustering
%         (default ``'yes'``).
%     cfg.computeROI (str): ``'yes'``/``'no'`` per-ROI clustering
%         (default ``'yes'``).
...
%         before clustering_coef_wu), ``'binary'`` (thresholds at > 0 and
%         uses clustering_coef_bu), or ``'mst'`` (same thresholding as
%         'binary', matching the original gr_calculateClusteringWs
...
%
% Returns:
%     clus (struct): Struct with fields ``table`` (columns Subject, Wave,
%         Condition, Region, plus one column per frequency band with the
%         mean clustering coefficient), ``conditions``, ``ROI``, and
%         ``bands``. Empty (``[]``) if output already exists and
%         ``cfg.overwrite`` is ``'no'``.

%% ensure clustering_coef_bu/clustering_coef_wu (FieldTrip's bundled BCT) is on the path
ft_hastoolbox('BCT', 1);
...
if ~computeGlobal && ~computeROI
    error('gr_calculateClusteringWs: both computeGlobal and computeROI are off; nothing to compute')
end

if ~isempty(conditionLabelsList) && length(conditionLabelsList) ~= length(conditions)
    error(['gr_calculateClusteringWs: cfg.conditionLabels must have the same number of ' ...
...
if computeROI
    if ~quiet
        fprintf(['\t note: ROI-level clustering is computed on small subgraphs ' ...
...
%% compute clustering per condition x region x band
Subject   = {};
Wave      = {};
Condition = {};
Region    = {};
...
if ~quiet; fprintf('\t calculating clustering coefficient ... '); end
    ...
    for r = 1:numel(regionNames)
        ...
        for b = 1:nfreq
            ...
            try
                switch edgeType
                    case 'binary'
                        clusPerBand(b) = mean(clustering_coef_bu(W > 0));
                    case 'weighted'
                        clusPerBand(b) = mean(clustering_coef_wu(gr_normalizeW(W)));
                    case 'mst'
                        clusPerBand(b) = mean(clustering_coef_bu(W > 0));
                end
            catch ME
                warning('gr_calculateClusteringWs:%s %s/%s band %s: %s', ...
        ...
        band_vals(end+1,:) = clusPerBand;      %#ok<AGROW>
    end
end
...
if strcmpi(saveData, 'yes')
    ...
    if ~quiet
        ...
        save(subjectdata.PATHS.(fieldname), 'clus')
        fprintf('done \n')
